...
3. **正文摘要（长文/结构性文章）** — 对于含多个章节的长篇文章（如 Binance Research 报告），在核心摘要后还应为每个主要章节提炼 1-2 句要点，保留原文的结构逻辑
...
- 如果 Jina 返回 "Max challenge attempts exceeded" 类 WAF 拦截信息：
  ...
  - **Binance Research PDF 下载模式**：对于 Binance 的文章，WAF 严重时可直接从页面点击 "DOWNLOAD THE FULL REPORT" 按钮（或搜索 PDF 链接），通常指向 `https://public.bnbstatic.com/static/files/research/{slug}.pdf`。下载后使用 `pdfminer.six`（`pip install pdfminer.six`）提取文本
  - 对于 Binance 文章，en-JP 域名容易触发 WAF，优先尝试 en 域名（如 binance.com/en/... 而非 en-JP）
  - 如果浏览器也返回空页面（AWS WAF/Cloudflare 拦截），Google 缓存和 Wayback Machine 也经常被同一套 WAF 拦截，PDF 直链是更可靠的方案
